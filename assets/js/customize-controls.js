/* global twinstarBgColors, twinstarColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( twinstarBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( twinstarBgColors[ context ].color ) {
						backgroundColorValue = twinstarBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( twinstarBgColors[ context ].setting ).get();
					}
					twinstarSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( twinstarBgColors ).forEach( function( context ) {
			wp.customize( twinstarBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					twinstarSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		twinstarSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				twinstarSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function twinstarSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = twinstarColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function twinstarSetRetineLogoVisibility( visible ) {
